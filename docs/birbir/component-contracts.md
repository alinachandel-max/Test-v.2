...
| `search_header_shell` | pending | BirBir screen pattern | `prototype-library.css` + `index.html` | gutter `16`, radius `16`, input `body/primary/400` | base, filled, sticky-down, sticky-up | отдельный header per screen без канона |
...
| `full_filters_page_shell` | pending | BirBir screen pattern | `prototype-library.css` + `index.html` | header top spacing, white cards on gray page | default | bottomsheet-style shell, боковые белые полосы |
| `filter_bottomsheet_shell` | pending | BirBir screen pattern | `prototype-library.css` + `index.html` | bottom-attached, content-height aware, footer safe-area | default, searchable, dual footer | vertical centering, fixed height without content fit |
...
| `home_recent_search_cards` | `BirBir-App 60318:99525` | BirBir home screen pattern | `index.html` + `prototype-library.css` | card `240x58`, gap `6`, radius `16`, meta `12/500`, title `14/600` | horizontal rail | old image-stack recent card, close badge, width drift |
| `home_collections_rail` | `BirBir-App 60198:190935` | BirBir home screen pattern | `index.html` + `prototype-library.css` | square cards `100`, partial visible item `50x100`, gap `8`, radius `12.308` | first-viewport clipped rail | stretching partial banner to 100px, legacy collection SVGs |
| `home_product_card` | `BirBir-App 60198:190957` | BirBir card preview pattern | `birbir-prototype-library.js` + `prototype-library.css` | card width `175`, wide `358`, media radius `16`, content gap `6`, meta label/location/date | default, similar, badges, installment, wide CTA | static per-card markup, remote Figma asset URLs, missing `Новое` meta row |
| `detail_seller_tags` | `BirBir-App 55604:127305` | BirBir detail pattern | `index.html` + `prototype-library.css` | green tonal tag, chevron, canonical icons | `ID verified`, `PRO` | `Agency` / `Store` fallback text in detail |
| `detail_product_hero_card` | `BirBir-App 61571:106543` | BirBir detail pattern | `index.html` + `prototype-library.css` + `birbir-prototype-library.js` | 358 square media, radius `20`, outside photo dots, price `24/700`, title `16/600`, 28px tags | default, old price, delivery, bargain | image dots overlayed on photo, seller preview inside hero, similar-search pill over hero photo |
